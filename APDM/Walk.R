...
for (x in GHIfiles){
  ...
  fileidx <- grep("WalkAlphabet",files) 
  filepath <- paste0(GHIpath,"/",x,"/",files[fileidx-1])
...
for (x in TCfiles){
  ...
  files <- list.files(path)
  fileidx <- grep("WalkAlphabet",files) 
  filepath <- paste0(TCpath,"/",x,"/",files[fileidx-1])
  f <- read.csv(filepath, stringsAsFactors = F, skip = 9)
  # check condition
  condition <- read.csv(filepath, stringsAsFactors = F)
  print(condition[3,2])
...
for (x in WSfiles){
  ...
  files <- list.files(path)
  if (x == "IW8WS") {
    filepath <- paste0(WSpath,"/",x,"/",files[23])
  }
  else{
    filepath <- paste0(WSpath,"/",x,"/",files[22])
  }
  f <- read.csv(filepath, stringsAsFactors = F, skip = 9)
  # check condition
  condition <- read.csv(filepath, stringsAsFactors = F)
