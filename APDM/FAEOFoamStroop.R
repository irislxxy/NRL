...
for (x in GHIfiles){
  ...
  if (id == "GH19" | id == "GH20"){
    fileidx <- grep("FAEOFoamwstroop",files) 
    filepath <- paste0(GHIpath,"/",x,"/",files[fileidx])
  }
  else{
    filepath <- paste0(GHIpath,"/",x,"/",files[8])
  }
...
for (x in WSfiles){
  ...
  files <- list.files(path)
  if (x == "IW8WS") {
    filepath <- paste0(WSpath,"/",x,"/",files[18])
  }
  else{
    filepath <- paste0(WSpath,"/",x,"/",files[17])
  }
  f <- read.csv(filepath, stringsAsFactors = F, skip = 9)
  # check condition
  condition <- read.csv(filepath, stringsAsFactors = F)
...
write.csv(df, "FAEOFoamStroop.csv")